...
def seed_users_departments(
    db: Session,
) -> tuple[dict[str, Department], dict[str, User], list[User]]:
    pw = hash_password(
        data.DEMO_PASSWORD
    )  # one hash reused: seeding 54 users stays fast
    ...
    for name, _ in data.DEPARTMENTS:
        ...
        heads[name] = make_user(
            full, unique_email(full), Role.dept_head, name, gender, f"Head of {name}"
        )
...
def seed_master(db: Session, depts, demo) -> dict:
    ...
    for name, src, unit, value, scope, version, status in data.EMISSION_FACTORS:
        f = EmissionFactor(
            name=name,
            source_type=ERPType(src),
            unit=unit,
            factor_value=value,
            scope=Scope(scope),
            effective_date=dt.date(2026, 4, 1) if version > 1 else dt.date(2025, 4, 1),
            version=version,
    ...
    for name, rule, value, icon, desc in data.BADGES:
        b = Badge(
            name=name, rule_type=rule, rule_value=value, icon=icon, description=desc
        )
        out["badges"][name] = b
        db.add(b)
    ...
    for name, unit, baseline, target, current, dept, factor in data.GOALS:
        db.add(
            EnvironmentalGoal(
                ...
                owner_department_id=depts[dept].id,
                linked_factor_name=factor,
...
def seed_erp_carbon(db: Session, depts, demo, factors) -> int:
    ...
    combos = [
        ...
        (
            "Manufacturing",
            ("Process Energy (Manufacturing)", 1),
            ERPType.manufacturing,
            "Process energy",
            (5000, 9000),
            "kWh",
    ...
    def add_op(
        ...
        line = ERPOperationLine(
            operation_id=op.id, resource=resource, quantity=qty, unit=unit
        )
        db.add(line)
        db.flush()
        db.add(
        ...
        for u in rng.sample(all_users, int(len(all_users) * 0.6)):
            db.add(
                TrainingCompletion(
                    ...
                    user_id=u.id,
                    completed_at=_dtime(
                        dt.date(2026, rng.randint(5, 7), rng.randint(1, 11))
                    ),
                )
            )
    db.flush()
...
def _decide(
    db, part, decision, decider, comment, award_fn, notif_type, label, do_notify
):
    part.status = {
...
def seed_csr(db: Session, master, demo, employees) -> None:
    ...
    for i, title in enumerate(data.CSR_TITLES):
        if i < 10:
            status, start = CSRStatus.active, TODAY + dt.timedelta(
                days=rng.randint(3, 50)
            )
        elif i < 16:
            status, start = CSRStatus.completed, dt.date(
                2026, rng.randint(4, 6), rng.randint(1, 25)
            )
        elif i < 18:
            status, start = CSRStatus.draft, TODAY + dt.timedelta(
                days=rng.randint(30, 80)
            )
        else:
        ...
        a = CSRActivity(
            ...
            location=fake.city(),
            organizer_user_id=demo["esg"].id,
            capacity=rng.randint(15, 60),
            start_date=start,
            end_date=start + dt.timedelta(days=rng.randint(0, 2)),
            budget_inr=rng.randint(20, 200) * 1000,
            points=rng.choice([25, 50, 75, 100]),
            status=status,
            created_by=demo["esg"].id,
        )
        activities.append(a)
    ...
    for a in activities:
        ...
        if (
            a.status == CSRStatus.completed
            and demo["employee"] not in group
            and rng.random() < 0.7
        ):
            group.append(demo["employee"])
        for u in group:
            part = CSRParticipation(activity_id=a.id, user_id=u.id)
            part.created_at = _dtime(
                a.start_date - dt.timedelta(days=rng.randint(3, 20))
            )
            db.add(part)
            db.flush()
            ...
            if a.status == CSRStatus.completed:
                ...
                if roll < 0.7:
                    ...
                    _decide(
                        ...
                        "approve",
                        decider,
                        "Great work at the event!",
                        award_once_for_csr,
                        NotificationType.csr_decision,
                        a.title,
                        do_notify,
                    )
                elif roll < 0.8:
                    ...
                    _decide(
                        ...
                        "reject",
                        decider,
                        "Proof does not show event participation.",
                        award_once_for_csr,
                        NotificationType.csr_decision,
                        a.title,
                        do_notify,
                    ...
                    _decide(
                        ...
                        "resubmit",
                        decider,
                        "Please upload a clearer photo with the team.",
                        award_once_for_csr,
                        NotificationType.csr_decision,
                        a.title,
                        do_notify,
...
def seed_challenges(db: Session, master, demo, employees) -> None:
    challenges = []
    statuses = (
        [ChallengeStatus.active] * 4
        + [ChallengeStatus.under_review] * 2
        + [ChallengeStatus.completed] * 2
        + [ChallengeStatus.draft, ChallengeStatus.archived]
    )
    for (title, cat, xp, diff, evidence), status in zip(data.CHALLENGES, statuses):
        c = Challenge(
            title=title,
            category_id=master["ch_cats"][cat].id,
            description=fake.paragraph(4),
            xp=xp,
            difficulty=Difficulty(diff),
            evidence=EvidenceMode(evidence),
            deadline=(
                TODAY + dt.timedelta(days=rng.randint(20, 90))
                if status
                in (
    ...
    for c in challenges:
        ...
        if (
            ...
            group.append(demo["employee"])
        needs_proof = (
            c.evidence == EvidenceMode.required or c.evidence == EvidenceMode.inherit
        )
        for u in group:
            ...
            if c.status == ChallengeStatus.completed:
                ...
                if roll < 0.75:
                    if needs_proof:
                        part.proof_attachment_id = _proof(
                            db, u, "challenge_participation"
                        ).id
                    _decide(
                        db,
                        part,
                        ...
                        "Verified — well done!",
                        award_once_for_challenge,
                        NotificationType.challenge_decision,
                        c.title,
                        do_notify,
                    )
                elif roll < 0.85:
                    ...
                    _decide(
                        ...
                        "reject",
                        decider,
                        "Logs do not cover the full period.",
                        award_once_for_challenge,
                        NotificationType.challenge_decision,
                        c.title,
                        do_notify,
                    ...
                    _decide(
                        ...
                        "resubmit",
                        decider,
                        "Please include the before/after readings.",
                        award_once_for_challenge,
                        NotificationType.challenge_decision,
                        c.title,
                        do_notify,
...
def seed_redemptions(db: Session, master, demo, employees) -> None:
    ...
    plans = [
        ("Steel Water Bottle", "placed"),
        ("Canvas Tote Bag", "placed"),
        ("Plant a Tree in Your Name", "fulfilled"),
        ("Organic Snack Box", "fulfilled"),
        ("Steel Water Bottle", "cancelled"),
        ("Bamboo Desk Kit", "returned"),
        ("Plant a Tree in Your Name", "placed"),
        ("Canvas Tote Bag", "fulfilled"),
    ]
...
def seed_governance(db: Session, depts, demo) -> None:
    for title, severity, dept_name, key in data.COMPLIANCE_ISSUES:
        ...
        if key.startswith("overdue"):
            ...
            status = (
                IssueStatus.open if key == "overdue_open" else IssueStatus.in_progress
            )
            resolved_at = None
        elif key == "open":
            due, status, resolved_at = (
                TODAY + dt.timedelta(days=rng.randint(20, 60)),
                IssueStatus.open,
                None,
            )
        elif key == "in_progress":
            due, status, resolved_at = (
                TODAY + dt.timedelta(days=rng.randint(10, 45)),
    ...
    for title, dept_name, score, status in audit_rows:
        db.add(
            Audit(
                ...
                auditor_user_id=demo["esg"].id,
                scope_note=f"{dept_name} — processes, records and controls",
                department_id=depts[dept_name].id,
                audit_date=(
                    dt.date(2026, rng.randint(4, 6), rng.randint(3, 27))
...
def run() -> None:
    ...
    try:
        ...
        assign_badge_manual(
            db, demo["head"], master["badges"]["Founder's Award"], demo["admin"].id
        )
        assign_badge_manual(
            db, demo["employee"], master["badges"]["Green Innovator"], demo["admin"].id
        )
